...
| **Scheduled Triage** | **Hourly** | ~1 min | **~720 min** | **⚠️ DANGER**: Ensure `gemini-scheduled-triage.yml` is NOT installed if you have limited minutes. |
...
Different modes consume different amounts of tokens (and money).

| Feature | Mode | Cost Profile | Why? |
| :--- | :--- | :--- | :--- |
| **`/plan`** | **V1 (Context-Aware)** | **Low** (1 Call) | Generates a markdown list in one go. |
| **`/plan`** | **V2 (Strict)** | **High** (N+1 Calls) | 1 call to plan, plus **1 full context call per task** for execution. |
| **`/review`** | **Two-Stage** | **Variable** | Stops early if Spec Compliance fails (saving tokens). Full cost only if code is good. |
| **`/fix`** | **Advisory TDD** | **Standard** | Single fixes. |
| **`/fix`** | **Strict TDD** | **High** (2x-3x) | Requires "Red-Green-Refactor" cycle (prove failure -> fix -> prove success), consuming more turns. |

> [!TIP]
> **V2 Planning** and **Strict TDD** are powerful but expensive. Use them for complex tasks, not simple typo fixes.
